...
{ 
    ...
    { 
        ...
        { 
            for (int j = 0; j < hidenode; j++) 
            { 
                hiddenLayer[i][j] = new hiddenNode(); hiddenLayer[i][j]->bias = get_11Random(); 
        ...
        { outputLayer[i] = new outputNode(); outputLayer[i]->bias = get_11Random(); 
        } 
...
{ 
        ...
        { cout << "training error: " << error << endl; error = 0.f; 
...
{ 
    ...
    { testGroup[iter].out.clear(); setInput(testGroup[iter].in); 
    // forward propagation on hidden layer 
    for (int i = 0; i < hidelayer; i++) 
    { 
        if (i == 0) 
        { 
            ...
            { 
                ...
                sum += hiddenLayer[i][j]->bias; hiddenLayer[i][j]->value = sigmoid(sum); 
            } 
        } 
        else 
        { 
    ...
    { double sum = 0.f; 
        ...
        sum += outputLayer[i]->bias; outputLayer[i]->value = sigmoid(sum); 
